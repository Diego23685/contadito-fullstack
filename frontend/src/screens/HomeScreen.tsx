// src/screens/HomeScreen.tsx — Panel + IA + GiftedCharts + Tutorial anclado a botones (responsive fijo)
...
= ({ children, style }) => (
  <LinearGradient colors={['#ffffff', '#f7f8ff']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.card, { borderTopColor: BRAND.iris }, style]}>
    {children}
  </LinearGradient>
);
...
const Bar: React.FC<{ value: number; max: number }> = ({ value, max }) => {
  ...
  return <View style={styles.barTrack}><View style={[styles.barFill, { width: `${pct * 100}%` }]} /></View>;
};
...
const PanelRow: React.FC<{ label: string; value?: React.ReactNode }> = ({ label, value }) => (
  <View style={styles.panelRow}><Text style={styles.panelRowLabel}>{label}</Text><Text style={styles.panelRowValue} numberOfLines={1}>{value ?? '—'}</Text></View>
);
const SidePanel: React.FC<{
    ...
    <View style={[styles.panel, { width }]}>
      ...
      <View style={styles.panelBlock}>
        ...
        <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
          ...
          <SmallBtn title="Refrescar tablero" onPress={() => {}} />
        </View>
      </View>
  ...
  if (pinned) return <View style={[styles.panelPinnedWrap, { width }]}>{PanelContent}</View>;
  return (
    <>
...
export default function HomeScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    const id = setTimeout(measureAll, 300); // espera a layout
...
Estado: ${context}`;
      ...
      // Manejo de errores de Ollama (500/405/etc.)
      if (!res.ok) {
      ...
      if (!parsed) {
        // Si vino un error de backend embebido
  ...
  // Pasos con targetKey mapeado a refs reales (más cobertura)
  ...
  const requestScroll = useCallback((y: number) => {
    ...
    setTimeout(measureAll, 250); // re-medimos tras el scroll
  }, [measureAll]);
    ...
    <View style={{ flex: 1, backgroundColor: BRAND.surfaceTint }}>
      ...
      >
        ...
        <View style={[styles.headerWrap]}>
          <View style={[styles.container, styles.containerMax]}>
            <View style={[styles.headerInner, isNarrow && { flexDirection: 'column', alignItems: 'flex-start', gap: 8 }]}>
              ...
              {/* Botones en wrap para pantallas angostas */}
              <View style={[styles.headerBtnsWrap, isNarrow && styles.headerBtnsWrapNarrow]}>
                {!panelPinned && (
                  <View ref={btnPanelRef} style={{ flexShrink: 0 }}>
                    <SmallBtn title={panelOpen ? 'Cerrar panel' : 'Abrir panel'} onPress={() => { onTogglePanel(); setTimeout(measureAll, 200); }} />
                  </View>
                )}
        ...
        <View style={[styles.sectionSticky]}>
          <View style={[styles.container, styles.containerMax]}>
            {/* Fila 1: input + buscar/limpiar (con wrap) */}
            ...
            {/* Fila 2: acciones rápidas (wrap) */}
            <View style={styles.quickRow}>
              <View ref={btnVentaRef}><SmallBtn title="Venta" onPress={() => navigation.navigate('SaleCreate')} /></View>
              <View ref={btnCompraRef}><SmallBtn title="Compra" onPress={() => navigation.navigate('PurchaseCreate')} /></View>
              <View ref={btnProductoRef}><SmallBtn title="Producto" onPress={() => navigation.navigate('ProductForm')} /></View>
              <View ref={btnClienteRef}><SmallBtn title="Cliente" onPress={() => navigation.navigate('CustomerForm')} /></View>
            </View>
          </View>
        </View>
                ...
                <View style={[styles.grid, styles.cols3]}>
                  {(loadingFirst ? [0,1,2] : totals).map((k: any, idx: number) => (
                    <GradientCard key={k?.label ?? `skt-${idx}`} >
                      {loadingFirst ? (<><Skeleton width={90} /><Skeleton height={24} style={{ marginTop: 8, width: 60 }} /></>) : (<><Label muted style={{ marginBottom: 6 }}>{k.label}</Label><Text style={[styles.kpiValue, isXL && styles.kpiValueXL]}>{k.value}</Text></>)}
                    </GradientCard>
                  ))}
                </View>
              </Section>
                ...
                <View style={[styles.grid, styles.cols2]}>
                  ...
                  <Card>
                    ...
                    <LineChart
                      ...
                      startFillColor="#e6e8ff" endFillColor="#e6e8ff00" startOpacity={1} endOpacity={0.1}
                      thickness={2} color={BRAND.hanBlue}
...
const styles = StyleSheet.create({
  ...
  // NUEVO: contenedor de botones con wrap
  headerBtnsWrap: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 8,
  ...
  // Sticky por encima de contenido
  sectionSticky: { backgroundColor: BRAND.surfaceSubtle, borderBottomColor: BRAND.borderSoft, borderBottomWidth: 1, paddingVertical: 8, zIndex: 5, elevation: 5 },

  section: { paddingHorizontal: 16, paddingVertical: 12, backgroundColor: BRAND.surfaceSubtle },
  ...
  // Buscador y acciones rápidas (ajustado)
  ...
  searchInput: { ...F, borderWidth: 1, borderColor: BRAND.borderSoft, borderRadius: 10, paddingHorizontal: 12, minHeight: 42, paddingRight: 34, backgroundColor: BRAND.surfacePanel, fontSize: 16 },
  searchClear: { position: 'absolute', right: 10, top: 9, width: 24, height: 24, borderRadius: 12, alignItems: 'center', justifyContent: 'center', backgroundColor: BRAND.surfaceSubtle },
  ...
  cols3: {}, cols2: {},
  card: {
    flexGrow: 1, minWidth: 220, backgroundColor: BRAND.surfacePanel, borderRadius: 14, padding: 12,
    borderWidth: 1, borderColor: BRAND.borderSoft, borderTopWidth: 3, borderTopColor: BRAND.accent,
    shadowColor: BRAND.accent, shadowOpacity: 0.06, shadowRadius: 10, shadowOffset: { width: 0, height: 6 }, elevation: 3,
  },
  ...
  smallBtn: {
    paddingHorizontal: 12, paddingVertical: 10, borderRadius: 12, borderWidth: 1, borderColor: BRAND.borderSoft, backgroundColor: BRAND.surfacePanel,
    shadowColor: BRAND.accentAlt, shadowOpacity: 0.03, shadowRadius: 8, shadowOffset: { width: 0, height: 3 }, elevation: 1,
    flexShrink: 0, alignSelf: 'flex-start',
  },
  smallBtnDanger: { backgroundColor: '#fff1f2', borderColor: '#fecdd3' },
  smallBtnText: { ...F, color: BRAND.hanBlue },

  panelPinnedWrap: { position: 'absolute', left: 0, top: 0, bottom: 0, backgroundColor: BRAND.surfacePanel, borderRightColor: BRAND.borderSoft, borderRightWidth: 1, zIndex: 10 },
  panelDrawer: { position: 'absolute', left: 0, top: 0, bottom: 0, backgroundColor: BRAND.surfacePanel, borderRightColor: BRAND.borderSoft, borderRightWidth: 1, zIndex: 20 },
  ...
  avatar: { width: 44, height: 44, borderRadius: 22, backgroundColor: BRAND.maximumBlue, alignItems: 'center', justifyContent: 'center' },
  avatarText: { ...F, color: '#ffffff' },
  panelTitleMain: { ...F, color: '#0f172a' },
  panelSubtitle: { ...F, color: '#6f7b94', fontSize: 12, marginTop: 2 },
  panelCard: { backgroundColor: BRAND.surfacePanel, borderWidth: 1, borderColor: BRAND.borderSoft, borderRadius: 14, padding: 12, marginBottom: 12, borderTopWidth: 3, borderTopColor: BRAND.accentAlt },
  panelCardTitle: { ...F, color: '#0f172a' },
  panelCardSub: { ...F, color: '#6f7b94', marginTop: 6 },
  panelBlock: { marginBottom: 14 },
  panelBlockTitle: { ...F, fontSize: 11, color: BRAND.darkPastelBlue, marginBottom: 8, textTransform: 'uppercase', letterSpacing: 0.6 },
  panelRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12, paddingVertical: 6, borderBottomColor: BRAND.borderSofter, borderBottomWidth: 1 },
  panelRowLabel: { ...F, color: '#6b7280', width: 120, fontSize: 12 },
  panelRowValue: { ...F, color: '#0f172a', flex: 1 },
  linkLike: { ...F, color: BRAND.hanBlue },
  linkChip: { paddingHorizontal: 10, paddingVertical: 8, borderRadius: 999, borderWidth: 1, borderColor: BRAND.borderSoft, backgroundColor: BRAND.surfacePanel, shadowColor: BRAND.accent, shadowOpacity: 0.02, shadowRadius: 6, shadowOffset: { width: 0, height: 2 } },
