...
const CustomersList: React.FC<any> = ({ navigation }) => {
  ...
  const load = useCallback(async (reset: boolean) => {
    ...
    try {
      ...
      const res = await api.get<CustomersResponse>('/customers', { params: { page: nextPage, pageSize: PAGE_SIZE, q: q || undefined } });
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#fff' }}>
      <View style={styles.toolbar}>
        <TextInput style={styles.input} placeholder="Buscar por nombre o email" value={q} onChangeText={setQ} />
        <Button title="Buscar" onPress={() => load(true)} />
        <Button title="Nuevo" onPress={() => navigation.navigate('CustomerForm')} />
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.item}>
            ...
            <Button title="Editar" onPress={() => navigation.navigate('CustomerForm', { id: item.id })} />
            <View style={{ width: 8 }} />
